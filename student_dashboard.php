    ...
    if ($file['error'] === 0 && $file['size'] < 5000000) {
        ...
        if (in_array($mime, $allowed)) {
            ...
            if (move_uploaded_file($file['tmp_name'], $upload_dir.$filename)) {
                ...
                $upd->bind_param("si",$web_path,$student['id']); $upd->execute(); $upd->close();
                $stmt2=$conn->prepare("SELECT * FROM users WHERE id=?");
                $stmt2->bind_param("i",$_SESSION['student_id']); $stmt2->execute();
                $student=$stmt2->get_result()->fetch_assoc(); $stmt2->close();
            }
        }
    }
...
$history_count=$hq->get_result()->fetch_assoc()['cnt']??0; $hq->close();

// Active sit-in
$aq=$conn->prepare("SELECT * FROM sit_in WHERE idnumber=? AND status='active' LIMIT 1");
$aq->bind_param("s",$student['idnumber']); $aq->execute();
$active_sitin=$aq->get_result()->fetch_assoc(); $aq->close();

// Sit-in summary stats
$sq=$conn->prepare("SELECT COALESCE(SUM(TIMESTAMPDIFF(MINUTE,session_date,time_out)),0) as tm, COUNT(*) as tc FROM sit_in WHERE idnumber=? AND status='done' AND time_out IS NOT NULL");
$sq->bind_param("s",$student['idnumber']); $sq->execute();
$srow=$sq->get_result()->fetch_assoc(); $sq->close();
$totalMins=(int)$srow['tm']; $totalSess=(int)$srow['tc'];
function fmtM($m){ if($m<=0)return'0m'; $h=floor($m/60); $r=$m%60; return $h>0?"{$h}h {$r}m":"{$r}m"; }

// Reservation enabled check (safe — table may not exist yet)
$resEnabled = true;
...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    :root{--navy:#0f2a55;--blue:#1a5276;--mid:#2563c0;--light:#3b82f6;--bg:#eef2f7;--card:#fff;--border:#d0daea;--sidebar:#162d55;}
    html,body{height:100%;}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:#1e293b;display:flex;flex-direction:column;min-height:100vh;}

    /* HEADER */
    .site-header{background:linear-gradient(90deg,#0a2744,#1a4fa0);display:flex;align-items:center;padding:10px 20px;gap:14px;box-shadow:0 2px 12px rgba(0,0,0,0.35);flex-shrink:0;}
    ...
    /* TOP NAV */
    .top-nav{background:var(--navy);display:flex;align-items:stretch;padding:0 12px;border-bottom:3px solid #071d3a;flex-shrink:0;flex-wrap:wrap;}
    .top-nav a{color:#a8c4e8;text-decoration:none;font-size:12.5px;font-weight:600;padding:10px 12px;white-space:nowrap;display:flex;align-items:center;gap:5px;border-bottom:3px solid transparent;margin-bottom:-3px;transition:color 0.15s,border-color 0.15s,background 0.15s;}
    ...
    /* 3-COLUMN */
    .page-body{display:grid;grid-template-columns:272px 1fr 280px;flex:1;overflow:hidden;}

    /* LEFT PANEL */
    .left-panel{background:var(--sidebar);display:flex;flex-direction:column;border-right:2px solid #071d3a;overflow-y:auto;}
    .panel-title{background:linear-gradient(135deg,#1234a0,var(--mid));color:#fff;padding:11px 16px;font-size:13px;font-weight:700;letter-spacing:0.04em;border-bottom:2px solid #071d3a;flex-shrink:0;}

    .avatar-section{display:flex;flex-direction:column;align-items:center;padding:22px 16px 12px;gap:10px;}
    .avatar-ring{width:110px;height:110px;border-radius:50%;border:3px solid rgba(255,255,255,0.18);background:#1e3f7a;display:flex;align-items:center;justify-content:center;font-size:44px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.3);}
    .avatar-ring img{width:100%;height:100%;object-fit:cover;}
    .student-fullname{color:#fff;font-weight:700;font-size:14px;text-align:center;line-height:1.3;}
    .student-course-tag{background:rgba(255,255,255,0.12);color:#b8d0f5;font-size:11px;font-weight:600;padding:3px 12px;border-radius:20px;text-align:center;}
    .photo-row{display:flex;gap:6px;align-items:center;justify-content:center;flex-wrap:wrap;}
    .btn-choose{background:rgba(37,99,192,0.7);color:#fff;padding:5px 12px;border-radius:5px;cursor:pointer;font-size:11px;white-space:nowrap;border:1px solid rgba(255,255,255,0.2);}
    .btn-upload{background:rgba(39,174,96,0.8);color:#fff;border:none;padding:5px 12px;border-radius:5px;cursor:pointer;font-size:11px;}

    .divider{height:1px;background:rgba(255,255,255,0.07);margin:0 16px;}
    .info-section{padding:10px 16px 14px;}
    .info-row{display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);}
    ...
    .session-display{display:inline-flex;align-items:center;gap:8px;margin-top:3px;}
    ...
    .active-banner{background:linear-gradient(135deg,#065f46,#059669);color:#fff;padding:10px 18px;font-size:12.5px;font-weight:600;display:flex;align-items:center;gap:8px;flex-shrink:0;}
    ...
    /* RESPONSIVE */
    @media(max-width:1024px){.page-body{grid-template-columns:240px 1fr 240px;}}
    @media(max-width:768px){.page-body{grid-template-columns:1fr;overflow:visible;height:auto;}.left-panel,.mid-panel,.right-panel{overflow:visible;border:none;border-bottom:1px solid var(--border);min-height:unset;}body{overflow:auto;}.page-body{display:block;}}
    @media(max-width:480px){.site-header{padding:8px 12px;gap:8px;}.site-header img{height:36px;}.top-nav a{font-size:11.5px;padding:9px 9px;}}
  </style>
</head>
<body>
...
<div class="page-body">
  ...
  <aside class="left-panel">
    ...
    <div class="avatar-section">
      ...
      <div class="student-fullname"><?= htmlspecialchars($student['firstname'].' '.$student['lastname']) ?></div>
      <div class="student-course-tag"><?= htmlspecialchars($student['course']) ?> · Year <?= $student['courselevel'] ?></div>
      <form method="POST" enctype="multipart/form-data">
        <input type="file" id="photoFile" name="photo" accept="image/*" style="display:none" onchange="document.getElementById('photoLbl').textContent=this.files[0]?.name?.substring(0,13)+'…'||'📷 Choose'"/>
    ...
    <div class="info-section">
      <div class="info-row"><span class="info-icon">🪪</span><div><span class="info-label">ID Number</span><span class="info-value"><?= htmlspecialchars($student['idnumber']) ?></span></div></div>
      <div class="info-row"><span class="info-icon">✉️</span><div><span class="info-label">Email</span><span class="info-value"><?= htmlspecialchars($student['email']) ?></span></div></div>
      <div class="info-row"><span class="info-icon">📍</span><div><span class="info-label">Address</span><span class="info-value"><?= htmlspecialchars($student['address']??'—') ?></span></div></div>
      <div class="info-row"><span class="info-icon">⏱️</span><div><span class="info-label">Remaining Sessions</span><div class="session-display"><span class="session-num"><?= $sess_left ?></span><span class="session-label">session<?= $sess_left!==1?'s':'' ?> left</span></div></div></div>
  ...
  <section class="mid-panel">
    ...
    <div class="ann-list">
      <?php if ($announcements && $announcements->num_rows > 0):
        ...
        <div class="ann-item">
          <div class="ann-meta"><span class="ann-dot"></span>CCS <?= htmlspecialchars($ann['posted_by']) ?> &nbsp;·&nbsp; <?= date('M d, Y', strtotime($ann['created_at'])) ?></div>
          <div class="ann-body"><?= nl2br(htmlspecialchars($ann['content'])) ?></div>
        </div>
      <?php endwhile; else: ?>
        <div class="no-ann"><div style="font-size:36px;margin-bottom:10px;">📭</div>No announcements at this time.</div>
      <?php endif; ?>
    </div>
  </section>
  ...
  <aside class="right-panel">
    ...
    <div class="rules-body">
      <div class="rules-school"><div class="school-name">University of Cebu</div><div class="college-name">COLLEGE OF INFORMATION &amp; COMPUTER STUDIES</div></div>
